...
{
  ctx.pos = 0;
  if (psf_load(filename.c_str(), &psf_file_system, 0x24, nullptr, nullptr, psf_info_meta, &ctx, 0,
               sf_status, nullptr) <= 0)
    return false;

  if (psf_load(filename.c_str(), &psf_file_system, 0x24, twosf_loader, &ctx.state, twosf_info,
               &ctx.state, 1, sf_status, nullptr) < 0)
    return false;
...
bool C2SFCodec::ReadTag(const std::string& file, kodi::addon::AudioDecoderInfoTag& tag)
{
  ...
  if (psf_load(file.c_str(), &psf_file_system, 0x24, nullptr, nullptr, psf_info_meta, &tsf, 0,
               sf_status, nullptr) <= 0)
  {
    return false;
  }
